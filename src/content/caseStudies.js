export const caseStudies = [
  ...
  },
  {
    ...
    sections: {
      ...
      decisions: [
        'Went with one shared database and a venue column on every row. The reason a shop pays for this rather than buying something off the shelf is that a driver lap follows them to any Oasis: fastest Spa time across the whole network, season standings that roll up nationally. In a shared database that is the query I already run with the venue filter left off, on indexes that already exist.',
        ...
        'Answered the isolation question with an application boundary, and with row level security in the database if a franchisee wants it in writing. What is in the schema is a display name, a hashed four-digit PIN, and lap times. No driver email addresses, no payment data. The strongest case for a database per tenant is regulatory or contractual, and a franchisee real worry is commercial: can the shop across town see my customer list.',
        'Left the split available rather than ruled out. If it becomes correct later, the venue column is the seam it partitions on.',
      ],
      tradeoffs: [
        'Separate databases make isolation physical rather than promised. In a shared database one missed WHERE clause leaks a franchise data to another franchise. In separate databases a bug cannot cross a connection boundary, because there is no connection to cross.',
